...
const App: React.FC = () => {
  // 1. 페이지 위치 기억: 로컬스토리지에서 마지막 앱 ID 로드
  const [currentApp, setCurrentApp] = useState<AppId>(() => {
    return (localStorage.getItem(LAST_APP_KEY) as AppId) || AppId.HOME;
  });
  ...
  const handleCloudSync = useCallback(async (action: 'push' | 'pull', targetId?: string) => {
    ...
    try {
      ...
      if (action === 'push') {
        ...
        // npoint.io를 통한 데이터 백업 (ID가 유효해야 함)
        ...
        <div className="flex items-center gap-2">
          {syncId && (
            <button 
              ...
              className={`p-2 rounded-lg transition-all ${syncStatus === 'syncing' ? 'animate-spin text-pink-400' : syncStatus === 'success' ? 'text-emerald-400' : 'text-white/60'}`}
              title="Quick Sync"
            >
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-t border-white/5 space-y-3">
            ...
            <button 
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-colors text-left border border-transparent hover:border-white/5"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-white/40 truncate">{syncId ? `ID: ${syncId}` : '로컬 전용 모드'}</p>
              </div>
      ...
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden relative">
        <header className="hidden md:flex h-20 bg-black/20 backdrop-blur-md border-b border-white/5 px-8 items-center justify-between z-10 sticky top-0">
           <div>
              <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                {activeAppDef?.name}
                <span className="text-[10px] px-2 py-0.5 rounded-full border border-pink-500/30 text-pink-400 bg-pink-500/10 uppercase tracking-wider font-black">ACTIVE</span>
              </h2>
           ...
           <div className="flex items-center gap-6">
              {syncId && (
                <div className={`flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-md transition-all ${syncStatus === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : syncStatus === 'error' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-white/5 border-white/10 text-white/50'}`}>
                   ...
                   <span className="text-xs font-black uppercase tracking-widest">
                     {syncStatus === 'syncing' ? 'Syncing...' : syncStatus === 'success' ? 'Data Saved' : syncStatus === 'error' ? 'Sync Error' : 'Cloud Link Active'}
                   </span>
                </div>
              )}
              ...
              <button 
                ...
                className="relative p-2.5 bg-white/5 rounded-full hover:bg-white/10 border border-white/10 text-white/60 hover:text-pink-400 transition-all active:scale-95 disabled:opacity-20 backdrop-blur-md group"
                title="데이터 즉시 저장"
              >
                 <RefreshCw className={`w-5 h-5 ${syncStatus === 'syncing' ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'}`} />
              </button>
           </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scroll-container p-4 md:p-8">
           <div className="max-w-6xl mx-auto pb-24 md:pb-10">
              <Suspense fallback={<div className="flex items-center justify-center h-64"><div className="flex flex-col items-center gap-3"><Loader2 className="w-10 h-10 animate-spin text-pink-500" /><p className="text-sm font-medium text-white/50 animate-pulse font-black uppercase tracking-widest">Loading Component</p></div></div>}>
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="bg-gradient-to-r from-violet-600/80 to-pink-600/80 backdrop-blur-md rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl border border-white/20 relative overflow-hidden group">
         <div className="relative z-10">
            <h1 className="text-3xl md:text-5xl font-black mb-3 tracking-tighter italic drop-shadow-md uppercase">Welcome back, Boss.</h1>
            <p className="text-white/90 max-w-xl text-sm md:text-lg leading-relaxed font-medium">
              모든 아카이브와 기록이 안전하게 보호되고 있습니다.<br className="hidden md:block"/>
              오늘의 목표를 달성하러 가볼까요? ✨
            </p>
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
         <div onClick={() => onChangeApp(AppId.CAR)} className="cursor-pointer bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white/10 active:scale-[0.98] md:hover:-translate-y-1 md:hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 group">
            ...
            <h3 className="font-black text-white/40 text-[10px] uppercase tracking-widest">Vehicle Status</h3>
            <p className="text-3xl md:text-4xl font-black text-white mt-2 tracking-tighter italic">DRIVE SAFE</p>
         </div>
         ...
         <div onClick={() => onChangeApp(AppId.GAGEBU)} className="cursor-pointer bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white/10 active:scale-[0.98] md:hover:-translate-y-1 md:hover:border-orange-500/50 hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-300 group">
            ...
            <h3 className="font-black text-white/40 text-[10px] uppercase tracking-widest">Expenses</h3>
            <p className="text-3xl md:text-4xl font-black text-white mt-2 tracking-tighter italic font-mono">FINANCE</p>
         </div>
         ...
         <div onClick={() => onChangeApp(AppId.FINANCE)} className="cursor-pointer bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white/10 active:scale-[0.98] md:hover:-translate-y-1 md:hover:border-green-500/50 hover:shadow-2xl hover:shadow-green-500/10 transition-all duration-300 group">
            ...
            <h3 className="font-black text-white/40 text-[10px] uppercase tracking-widest">Portfolio</h3>
            <p className="text-3xl md:text-4xl font-black text-white mt-2 tracking-tighter italic font-mono">MARKETS</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div onClick={() => onChangeApp(AppId.IDEAL_TYPE)} className="cursor-pointer bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white/10 active:scale-[0.98] md:hover:-translate-y-1 md:hover:border-pink-500/50 hover:shadow-2xl hover:shadow-pink-500/10 transition-all duration-300 group">
          <div className="flex justify-between items-start mb-6">
            ...
            <span className="text-[10px] font-black text-pink-300 bg-pink-500/10 px-3 py-1 rounded-full border border-pink-500/20 uppercase tracking-widest">Personal Archive</span>
          </div>
          <h3 className="font-black text-white/40 text-[10px] uppercase tracking-widest mb-1">Muses & Icons</h3>
          <p className="text-3xl font-black text-white tracking-tighter italic uppercase">Ideal Gallery</p>
        </div>
        ...
        <div onClick={() => onChangeApp(AppId.GOALS)} className="cursor-pointer bg-black/40 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-white/10 active:scale-[0.98] md:hover:-translate-y-1 md:hover:border-red-500/50 hover:shadow-2xl hover:shadow-red-500/10 transition-all duration-300 group">
          ...
          <h3 className="font-black text-white/40 text-[10px] uppercase tracking-widest">Ambitions</h3>
          <div className="mt-4 w-full bg-white/10 h-3 rounded-full overflow-hidden border border-white/5">
              <div className="bg-gradient-to-r from-red-500 to-orange-500 h-full rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(239,68,68,0.5)]" style={{ width: '45%' }}></div>
          </div>
          <p className="text-[10px] font-black text-white/30 mt-3 text-right uppercase tracking-widest">Processing Goals...</p>
        </div>
      </div>

      {/* Notifications & AI */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 pb-20">
          <div className="lg:col-span-2 bg-black/40 backdrop-blur-md rounded-[2.5rem] p-6 md:p-8 border border-white/10">
             <h3 className="text-lg font-black italic mb-6 text-white flex items-center gap-2 uppercase tracking-tighter"><Bell className="w-5 h-5 text-pink-500" /> System Log</h3>
             <div className="space-y-3">
                {[
                  { title: 'Welcome to your personalized LifeOS dashboard.', time: 'System', type: 'info', icon: Sparkles, color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' },
                  { title: 'All your galleries and finance data are stored in local DB.', time: 'Local', type: 'info', icon: CreditCard, color: 'text-pink-400 bg-pink-500/10 border-pink-500/20' },
                ].map((notif, i) => (
                   <div key={i} className="flex items-start gap-4 p-4 hover:bg-white/5 rounded-3xl transition-colors group cursor-pointer border border-transparent hover:border-white/5">
                      ...
                      <div className="flex-1 py-1">
                         <p className="text-sm font-bold text-white/90 group-hover:text-pink-400 transition-colors uppercase tracking-tight italic">{notif.title}</p>
                         <p className="text-[10px] font-black text-white/20 mt-1 uppercase tracking-widest">{notif.time}</p>
                      </div>
                   </div>
                ))}
             </div>
          ...
          <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 backdrop-blur-md rounded-[2.5rem] p-6 md:p-8 border border-indigo-500/20 flex flex-col justify-center items-center text-center relative overflow-hidden min-h-[280px]">
             ...
             <h3 className="font-black text-xl text-white mb-2 relative z-10 uppercase italic tracking-tighter">AI Assistant</h3>
             <p className="text-xs text-white/60 mb-8 leading-relaxed max-w-xs relative z-10 uppercase font-bold tracking-widest">Ask Moka anything about your life schedule.</p>
             <button onClick={() => onChangeApp(AppId.AI_ASSISTANT)} className="w-full bg-white text-black hover:bg-pink-100 px-6 py-4 rounded-2xl text-xs font-black transition-all shadow-lg active:scale-95 relative z-10 flex items-center justify-center gap-2 uppercase">
               <Sparkles className="w-4 h-4 text-pink-500" /> Start AI Chat
             </button>
          </div>
      </div>
    </div>
