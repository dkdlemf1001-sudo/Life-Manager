...
export const FinanceManager: React.FC = () => {
  ...
  useEffect(() => {
    const loadStocks = async () => {
      try {
          await db.init();
          const loadedStocks = await db.getAll<StockHolding>('stocks');
          setStocks(loadedStocks);
          const lastUpdate = await db.getSetting('stocks_last_update');
          setLastUpdated(lastUpdate || 'N/A');
      } catch (e) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (isUpdating || stocks.length === 0) return;
  ...
  const fetchRealtimePrices = async () => {
    ...
    try {
      ...
      await db.saveAll('stocks', updatedStocks);
      
      const timeStr = new Date().toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
      setLastUpdated(timeStr);
      await db.setSetting('stocks_last_update', timeStr);
  ...
  const handleSaveStock = async () => {
    ...
    const stockPayload = {
        ...
        avgPrice: Number(newStock.avgPrice) || 0
    };

    if (editingSymbol) {
      updatedStocks = stocks.map(s => s.symbol === editingSymbol ? {
        ...s,
        ...stockPayload
      } : s);
    } else {
      updatedStocks = [...stocks, {
          ...stockPayload,
          currentPrice: stockPayload.avgPrice // Initial price is buy price
      }];
    }
  ...
  if (loading) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
            ...
            <p className="text-white/60 font-bold">자산 포트폴리오 로딩 중...</p>
        </div>
      );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-24">
      {/* Premium Portfolio Header - Glassmorphic */}
      <div className="bg-black/40 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden border border-white/10">
        <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/10 rounded-full blur-[120px] -mr-32 -mt-32"></div>
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-emerald-500/10 rounded-full blur-[100px] -ml-20 -mb-20"></div>
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
            <div className="space-y-4">
              ...
              <div>
                <p className="text-white/60 text-sm font-medium mb-1">Estimated Net Worth</p>
                <div className="flex items-baseline gap-3">
                  <span className="text-5xl md:text-7xl font-mono font-bold tracking-tighter">
                    ${totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                  <span className="text-white/50 font-mono text-xl">USD</span>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3 w-full md:w-auto">
              <button 
                ...
                disabled={isUpdating}
                className="group relative overflow-hidden bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold py-4 px-8 rounded-2xl transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 backdrop-blur-md"
              >
              ...
              <button 
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 px-8 rounded-2xl shadow-xl shadow-indigo-500/20 flex items-center justify-center gap-3 transition-all active:scale-95 border border-indigo-500/50"
              >
      ...
      {groundingLinks.length > 0 && (
        <div className="px-2">
          <div className="bg-black/30 backdrop-blur-md rounded-2xl p-4 flex flex-wrap items-center gap-4 border border-white/5">
            ...
            <div className="flex flex-wrap gap-2">
              {groundingLinks.map((link, idx) => (
                <a key={idx} href={link.uri} target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold text-indigo-400 hover:underline flex items-center gap-1 bg-white/5 px-2 py-1 rounded-md border border-white/10 hover:bg-white/10 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Market Overview Chart - Glassmorphic */}
        <div className="lg:col-span-8 bg-black/40 backdrop-blur-md rounded-3xl p-8 shadow-sm border border-white/10 flex flex-col">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h3 className="font-bold text-white text-lg flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-indigo-500" /> Market Overview
              </h3>
              <p className="text-xs text-white/40 mt-1">Total portfolio value trend (24h simulation)</p>
            </div>
            <div className="flex bg-white/5 p-1 rounded-xl">
              {['1D', '1W', '1M', '1Y'].map(t => (
                <button key={t} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all ${t === '1D' ? 'bg-white/10 text-indigo-400 shadow-sm border border-white/5' : 'text-white/40 hover:text-white/80'}`}>{t}</button>
              ))}
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={mainChartData}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', color: '#fff', fontSize: '12px', backdropFilter: 'blur(8px)' }}
        ...
        {/* Portfolio Diversification (Pie Chart) - Glassmorphic */}
        <div className="lg:col-span-4 bg-black/40 backdrop-blur-md rounded-3xl p-8 shadow-sm border border-white/10">
           <h3 className="font-bold text-white text-lg flex items-center gap-2 mb-8">
           ...
           <div className="relative h-64 flex flex-col items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                 <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Weight</span>
                 <span className="text-xl font-mono font-bold text-white">100%</span>
              </div>
           </div>
           <div className="mt-8 grid grid-cols-2 gap-3">
              {stocks.map((stock, index) => (
                 <div key={stock.symbol} className="flex items-center gap-2.5 p-2 bg-white/5 rounded-xl border border-white/5">
                    <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: COLORS[index % COLORS.length] }}></div>
                    <div className="min-w-0 flex-1">
                       <p className="text-[11px] font-bold text-white/80 truncate">{stock.symbol}</p>
                       <p className="text-[9px] font-bold text-white/40 font-mono">
      ...
      {/* Asset Table View - Glassmorphic */}
      <div className="bg-black/40 backdrop-blur-md rounded-[2rem] shadow-sm border border-white/10 overflow-hidden">
        <div className="p-8 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="font-bold text-white text-xl tracking-tight">Active Portfolio Holdings</h3>
              <p className="text-xs text-white/40 mt-1 font-medium">Monitoring {stocks.length} assets in real-time</p>
            </div>
            <div className="flex items-center gap-2 text-xs font-bold text-white/60 bg-white/5 px-4 py-2 rounded-xl border border-white/5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-[10px] font-bold text-white/30 uppercase tracking-[0.15em] bg-white/5">
              ...
              {stocks.map((stock, idx) => {
                ...
                return (
                  <tr key={stock.symbol} className="group hover:bg-white/5 transition-all">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-white/40 font-bold uppercase tracking-wider">{stock.name}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <p className="font-mono font-bold text-white">${stock.currentPrice.toLocaleString()}</p>
                      <p className="text-[10px] text-white/40 font-medium">Avg: ${stock.avgPrice.toLocaleString()}</p>
                    </td>
                    <td className="px-8 py-6">
                      <div className="w-32">
                        <div className="flex justify-between text-[10px] font-bold mb-1.5 text-white/50">
                    ...
                    <td className="px-8 py-6">
                      <p className="font-mono font-bold text-white">${currentVal.toLocaleString()}</p>
                    </td>
                    <td className="px-8 py-6">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${gain >= 0 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
                        {gain >= 0 ? <ArrowUpRight className="w-3.5 h-3.5" /> : <ArrowDownRight className="w-3.5 h-3.5" />}
                        {gainPct.toFixed(2)}%
                      </div>
                      <p className={`text-[10px] font-bold mt-1 ml-1 ${gain >= 0 ? 'text-emerald-400/60' : 'text-rose-400/60'}`}>
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className="p-2.5 text-white/30 hover:text-blue-400 hover:bg-blue-500/10 rounded-xl transition-all"
                        >
      ...
      {/* Asset Modal (Add/Edit) - Glassmorphic */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-xl bg-black/60 animate-in fade-in duration-300">
           <div className="bg-black/50 backdrop-blur-2xl w-full max-w-lg rounded-[2.5rem] p-10 shadow-3xl border border-white/10 animate-in zoom-in-95 duration-300">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsModalOpen(false)} className="p-3 text-white/40 hover:bg-white/10 rounded-full transition-colors">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="space-y-6">
                 <div className="space-y-2">
                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] ml-1 block">Ticker Symbol</label>
                    <input 
                      type="text"
                      ...
                      className={`w-full bg-black/40 border border-white/10 rounded-2xl p-5 font-bold text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:font-normal placeholder:text-white/20 ${editingSymbol ? 'opacity-50 cursor-not-allowed' : ''}`}
                    />
                 </div>
                 
                 <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] ml-1 block">보유 수량 (Shares)</label>
                        <input 
                        type="number"
                        placeholder="0.00"
                        value={newStock.shares}
                        onChange={e => setNewStock({...newStock, shares: e.target.value})}
                        className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 font-bold text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-white/20"
                        />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] ml-1 block">평균 단가 (Avg Cost $)</label>
                        <input 
                        type="number"
                        placeholder="0.00"
                 ...
                 <div className="space-y-2">
                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] ml-1 block">자산 명칭 (Asset Name)</label>
                    <input 
                      type="text"
                      placeholder="e.g. Apple Inc."
                      value={newStock.name}
                      onChange={e => setNewStock({...newStock, name: e.target.value})}
                      className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 font-bold text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:font-normal placeholder:text-white/20"
                    />
                 </div>
              </div>
              ...
              <button 
                ...
                className="w-full mt-10 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-5 rounded-[1.5rem] shadow-xl shadow-indigo-500/20 transition-all active:scale-95 flex items-center justify-center gap-3 text-lg border border-indigo-500/30"
              >
